...
public class Action_OdekakeAsakusa : NgoEvent
{
	...
	private struct _003CstartEvent_003Ed__1 : IAsyncStateMachine
	{
		...
		public Action_OdekakeAsakusa _003C_003E4__this;
		...
		private void MoveNext()
		{
			...
			Action_OdekakeAsakusa action_OdekakeAsakusa = _003C_003E4__this;
			try
			{
				Awaiter val;
				UniTask val2;
				if (num != 0)
				{
					...
					action_OdekakeAsakusa.startEvent(cancellationToken, skipWatchSp: true);
					val2 = action_OdekakeAsakusa.GoOut();
				...
				SingletonMonoBehaviour<PoketterManager>.Instance.AddQueueWithKusoreps(TweetFetcher.CommandTweet(CommandType.Odekake_asakusa, CommandResult.success));
				SingletonMonoBehaviour<WindowManager>.Instance.NewWindow(AppType.Webcam);
				val2 = action_OdekakeAsakusa.BackFromOdekake();
				val = ((UniTask)(ref val2)).GetAwaiter();
				if (!((Awaiter)(ref val)).IsCompleted)
				{
					num = (_003C_003E1__state = 1);
				...
				action_OdekakeAsakusa.endEvent();
			}
