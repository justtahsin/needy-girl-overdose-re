...
public class Action_OdekakeToyosu : NgoEvent
{
	...
	private struct _003CstartEvent_003Ed__1 : IAsyncStateMachine
	{
		...
		public Action_OdekakeToyosu _003C_003E4__this;
		...
		private void MoveNext()
		{
			...
			Action_OdekakeToyosu action_OdekakeToyosu = _003C_003E4__this;
			try
			{
				Awaiter val;
				UniTask val2;
				if (num != 0)
				{
					...
					action_OdekakeToyosu.startEvent(cancellationToken, skipWatchSp: true);
					val2 = action_OdekakeToyosu.GoOut();
				...
				SingletonMonoBehaviour<PoketterManager>.Instance.AddQueueWithKusoreps(TweetFetcher.CommandTweet(CommandType.Odekake_toyosu, CommandResult.success));
				SingletonMonoBehaviour<WindowManager>.Instance.NewWindow(AppType.Webcam);
				val2 = action_OdekakeToyosu.BackFromOdekake();
				val = ((UniTask)(ref val2)).GetAwaiter();
				if (!((Awaiter)(ref val)).IsCompleted)
				{
					num = (_003C_003E1__state = 1);
