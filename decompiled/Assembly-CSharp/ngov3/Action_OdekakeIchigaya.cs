...
public class Action_OdekakeIchigaya : NgoEvent
{
	...
	private struct _003CstartEvent_003Ed__1 : IAsyncStateMachine
	{
		...
		public Action_OdekakeIchigaya _003C_003E4__this;
		...
		private void MoveNext()
		{
			...
			Action_OdekakeIchigaya action_OdekakeIchigaya = _003C_003E4__this;
			try
			{
				Awaiter val;
				UniTask val2;
				if (num != 0)
				{
					...
					action_OdekakeIchigaya.startEvent(cancellationToken, skipWatchSp: true);
					val2 = action_OdekakeIchigaya.GoOut();
				...
				SingletonMonoBehaviour<PoketterManager>.Instance.AddQueueWithKusoreps(TweetFetcher.CommandTweet(CommandType.Odekake_ichigaya, CommandResult.success));
				SingletonMonoBehaviour<WindowManager>.Instance.NewWindow(AppType.Webcam);
				val2 = action_OdekakeIchigaya.BackFromOdekake();
				val = ((UniTask)(ref val2)).GetAwaiter();
				if (!((Awaiter)(ref val)).IsCompleted)
				{
					num = (_003C_003E1__state = 1);
				...
				action_OdekakeIchigaya.endEvent();
			}
